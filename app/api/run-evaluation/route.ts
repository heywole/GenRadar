...
// This route does the actual GenLayer work — no timeout concern because
// we don't wait for it from the client side
...
export async function POST(req: NextRequest) {
  ...
  try {
    console.log(`[run-evaluation] Starting for ${project.name}`)
    ...
    // Delete old score and save new one
    ...
    console.log(`[run-evaluation] ✓ done: ${project.name} score=${aiScore.score}`)
    return NextResponse.json({ success: true, score: aiScore.score })

  } catch (err: any) {
    console.error(`[run-evaluation] ✗ failed: ${err.message}`)
    await supabase.from('projects')
